...
import RNNOptions from '../../NodeOptions/SpecificOptions/RNNOptions';
import propertyController from '../../../../controllers/propertyController';


const RNNNode = (props : NodeProps) =>{
    ...
    useEffect(() => {
        ...
        if(IncomingShape && units){
            if(IncomingShape.length === 2 && units >= 1)
            {
                ...
                setNeurons(units * ((IncomingShape[1] + units + 1)))//+1 if/when including bias
                setValid(true)
    ...
    const optionsMenu = <RNNOptions units = {units} setUnits = {setUnits} id = {id} seq2seq = {seq2seq} setSeq2Seq = {setSeq2Seq}/>;
